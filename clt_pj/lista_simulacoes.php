...
<?php 
    usort($simulacoesCltPj, function($a, $b) {
...
?>
	...
	<h2 class="text-center">Simulações CLT x PJ</h2>
	<div class="row">
		<div class="col">
			<?php if(count($simulacoesCltPj) > 0): ?>
			<table class="table">
                <thead>
                    <tr>
                        <th>COD CLT</th>
                ...
                <tbody>
				<?php foreach($simulacoesCltPj as $s): 
                    $dt = isset($s['dataSimulacao']) ? (new DateTime($s['dataSimulacao']))->format('d/m/y H:i:s') : '-';     
                ?>
		  		<tr>
                    <td>#<?= $s['codSimulacaoClt'] ?></td>
                    <td>#<?= $s['codSimulacaoPj'] ?></td>
                    <td><?= $s['salarioBruto'] ?></td>
                    <td><?= $s['faturamentoBruto'] ?></td>
                    <td><?= $dt ?></td>
                    <td>
						<a class="ml-3" href="index.php?codSimulacaoClt=<?= $s['codSimulacaoClt'] ?>&codSimulacaoPj=<?=$s['codSimulacaoPj']?>">
							Ver
						</a>
						<a class="ml-3" href="excluir.php?codSimulacaoClt=<?= $s['codSimulacaoClt'] ?>&codSimulacaoPj=<?=$s['codSimulacaoPj']?>">
							Excluir
						</a>
					</td>
                </tr>
