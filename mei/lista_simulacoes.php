...
<?php 
    usort($simulacoesMei, function($a, $b) {
...
?>
	...
	<h2 class="text-center">Simulações MEI</h2>
	<div class="row">
		<div class="col">
			<?php if(count($simulacoesMei) > 0): ?>
			<table class="table">
                <thead>
                    <tr>
                        <th>COD</th>
                ...
                <tbody>
				<?php foreach($simulacoesMei as $s): 
                    $dt = isset($s['dataSimulacao']) ? (new DateTime($s['dataSimulacao']))->format('d/m/y H:i:s') : '-';     
                ?>
		  		<tr>
                    <td>#<?= $s['form']['codSimulacaoMei'] ?></td>
                    <td><?= $s['form']['faturamento_bruto'] ?></td>
                    <td><?= $dt ?></td>
                    <td><a class="ml-3" href="index.php?codSimulacaoMei=<?= $s['form']['codSimulacaoMei'] ?>">Ver</a>
					<a class="ml-3" href="excluir.php?codSimulacaoMei=<?= $s['form']['codSimulacaoMei'] ?>">Excluir</a></td>
                </tr>
				<?php endforeach; ?>
                </tbody>
			</table>
