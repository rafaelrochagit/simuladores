...
class IRPF_PLR {
	...
	function salvar($tabela) {
		...
		file_put_contents($this->filepath."tabelas/".$id."_tabela_irpf_plr.json", $tabelaJson);
	}
	...
	function selecionar($tabelaNome) {
        ...
        $configuracoes['tabela_irpf_plr'] = $tabelaNome;
	...
	function aliquota($baseCalculo) {
		...
		$config = readConfig();
		$tabelaNome = isset($config['tabela_irpf_plr']) ? $config['tabela_irpf_plr'] : '';
