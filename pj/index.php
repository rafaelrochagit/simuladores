...
<div class="card">
	<div class="card-header text-center" style="font-size: 20pt;">
		Lucro Líquido Simples Nacional <small style="font-size: 11pt;"><?= date('Y') ?></small>
	</div>
	<div class="card-body">
			...
			<div class="row">
				...
				<div class="col-2">
					<div class="form-group text-left">
						<input id="codSimulacaoPj" class="form-control" type="number" name="codSimulacaoPj" value="<?= $codSimulacaoPj?>" readonly>
					</div>
				</div>
			</div>
			<h4 class="card-title text-left mb-5 d-print-none">Formulário</h4>
			<?php require_once 'form_pj.php' ?>
...
<script>
	function abrirSimulacao() {
		let cod = $("#codSimulacaoPj").val();
		let url_atual = window.location.origin + window.location.pathname;
		window.location.href = url_atual + '?codSimulacaoPj=' + cod
	}
