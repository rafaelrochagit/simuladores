...
<?php 
    usort($simulacoesPj, function($a, $b) {
...
?>
	...
	<h2 class="text-center">Simulações PJ</h2>
	<div class="row">
		<div class="col">
			<?php if(count($simulacoesPj) > 0): ?>
			<table class="table">
                <thead>
                    <tr>
                        <th>COD</th>
                ...
                <tbody>
				<?php foreach($simulacoesPj as $s): 
                    $dt = isset($s['dataSimulacao']) ? (new DateTime($s['dataSimulacao']))->format('d/m/y H:i:s') : '-';     
                ?>
		  		<tr>
                    <td>#<?= $s['form']['codSimulacaoPj'] ?></td>
                    <td><?= $s['form']['faturamento_bruto'] ?></td>
                    <td><?= $dt ?></td>
                    <td><a class="ml-3" href="index.php?codSimulacaoPj=<?= $s['form']['codSimulacaoPj'] ?>">Ver</a>
					<a class="ml-3" href="excluir.php?codSimulacaoPj=<?= $s['form']['codSimulacaoPj'] ?>">Excluir</a></td>
                </tr>
				<?php endforeach; ?>
                </tbody>
			</table>
