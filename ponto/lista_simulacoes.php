...
<?php 
    usort($simulacoesPonto, function($a, $b) {
...
?>
	...
	<h2 class="text-center">Simulações Ponto</h2>
	<div class="row">
		<div class="col">
			<?php if(count($simulacoesPonto) > 0): ?>
			<table class="table">
                <thead>
                    <tr>
                        <th>COD</th>
                ...
                <tbody>
				<?php foreach($simulacoesPonto as $s): 
                    $dt = isset($s['dataSimulacao']) ? (new DateTime($s['dataSimulacao']))->format('d/m/y H:i:s') : '-';     
                ?>
		  		<tr>
                    <td>#<?= $s['form']['codSimulacaoPonto'] ?></td>
                    <td><?= $s['horaInicial'] ?></td>
                    <td><?= $s['horaFinal'] ?></td>
                    <td><?= $s['timeTotal'] ?></td>
                    <td><?= $dt ?></td>
                    <td>
						<a class="ml-3" href="index.php?codSimulacaoPonto=<?= $s['form']['codSimulacaoPonto'] ?>">Ver</a>
						<a class="ml-3" href="excluir.php?codSimulacaoPonto=<?= $s['form']['codSimulacaoPonto'] ?>">Excluir</a>
					</td>
                </tr>
				<?php endforeach; ?>
                </tbody>
