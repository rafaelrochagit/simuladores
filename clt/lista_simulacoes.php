...
<?php 
    usort($simulacoesClt, function($a, $b) {
...
?>
	...
	<h2 class="text-center">Simulações CLT</h2>
	<div class="row">
		<div class="col">
			<?php if(count($simulacoesClt) > 0): ?>
			<table class="table">
                <thead>
                    <tr>
                        <th>COD</th>
                ...
                <tbody>
				<?php foreach($simulacoesClt as $s): 
                    $dt = isset($s['dataSimulacao']) ? (new DateTime($s['dataSimulacao']))->format('d/m/y H:i:s') : '-';     
                ?>
		  		<tr>
                    <td>#<?= $s['form']['codSimulacaoClt'] ?></td>
                    <td><?= $s['form']['salario_bruto'] ?></td>
                    <td><?= $dt ?></td>
                    <td><a class="ml-3" href="index.php?codSimulacaoClt=<?= $s['form']['codSimulacaoClt'] ?>">Ver</a>
					<a class="ml-3" href="excluir.php?codSimulacaoClt=<?= $s['form']['codSimulacaoClt'] ?>">Excluir</a></td>
                </tr>
				<?php endforeach; ?>
                </tbody>
			</table>
