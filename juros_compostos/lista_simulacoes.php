...
<?php 
    usort($simulacoesJc, function($a, $b) {
	...
	<div class="row">
		<div class="col">
			...
			<table class="table">
                ...
                <tbody>
				<?php foreach($simulacoesJc as $s): 
                    $dt = isset($s['dataSimulacao']) ? (new DateTime($s['dataSimulacao']))->format('d/m/y H:i:s') : '-';     
                ?>
		  		<tr>
                    <td>#<?= $s['form']['codSimulacaoJc'] ?></td>
                    <td><?= $s['form']['valor_inicial'] ?></td>
                    <td><?= $dt ?></td>
                    <td><a class="ml-3" href="index.php?codSimulacaoJc=<?= $s['form']['codSimulacaoJc'] ?>">Ver</a></td>
                    <td><a class="ml-3" href="excluir.php?codSimulacaoJc=<?= $s['form']['codSimulacaoJc'] ?>">Excluir</a></td>
                </tr>
				<?php endforeach; ?>
                </tbody>
			</table>
